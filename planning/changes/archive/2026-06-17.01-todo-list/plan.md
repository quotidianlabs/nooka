...
Run:
```bash
flutter create --org com.nooka --project-name nooka --platforms=android,ios .
```
...
```json
{
  ...
  "clearArchiveBody": "Permanently delete {count, plural, =0{no archived items} one{{count} archived item} other{{count} archived items}}?",
  "@clearArchiveBody": {
...
```json
{
  ...
  "clearArchiveBody": "Удалить из архива {count, plural, =0{нет дел} one{{count} дело} few{{count} дела} many{{count} дел} other{{count} дела}} навсегда?",
...
Run: `flutter test test/data/todo_dao_test.dart`
Expected: PASS (8 tests total). Implementation already exists from Task 11.
        ...
        actions: [
          if (_view == _View.archive)
            IconButton(
              ...
              onPressed: () => _clearArchive(state.valueOrNull ?? const []),
            ),
          IconButton(
...
> Note: the `switch (choice)` over the bottom-sheet result is a `String?` switch and is intentionally non-exhaustive (no `default`); `null` (sheet dismissed) falls through to no-op. If `flutter analyze` warns, add a trailing `case _: break;`.
...
Expected: PASS (6 tests). Fix any analyzer issues flagged by the note under Step 2 first.
...
> Note: `AppLocalizations.of(context)` inside `NookaApp.build` works because `MaterialApp` provides the delegate to its descendants; `onGenerateTitle` is evaluated below that. If analyzer flags `l10n` usage above the `MaterialApp`, move the title to a static string `'Nooka'` and keep `onGenerateTitle` for the localized value.
...
- **Spec coverage:** categories+color+emoji (Tasks 9, 15), items in one category (9), complete→archive (11/16), restore (11/16), 30-day auto-remove on startup + archive-open (7, 16, 18), light/dark (3, 17), en/ru with 4-form plurals (5, 17), grouped single screen with Active/Archive switcher (16), collapsible categories (9/16), cascade-delete with confirm (11/15/16), undo toast (16), keep-keyboard-open quick add + last-used category (15/16), swipe-to-complete with tap fallback + haptic (16), archive completion date + "clear archive now" (5/11/12/13/15/16), drag-reorder helper + DAO (8/11) — UI wiring of drag is deferred behind the tested reorder commands; accessibility color+name+emoji pairing (16), testing tiers (7/8/11/12/16/17/19).
